...
async def _compute_health(project_id: uuid.UUID, db: AsyncSession) -> dict:
    ...
    overdue_res = await db.execute(
        select(func.count()).where(
            and_(Task.project_id == project_id, Task.status != "done", Task.due_date < today)
        )
    )
    overdue = overdue_res.scalar() or 0
    overdue_ratio = round(overdue / total_open * 100, 2) if total_open > 0 else 0.0

    # Active members (updated task in last 7 days)
    total_members_res = await db.execute(
    ...
    active_members_res = await db.execute(
        select(func.count(func.distinct(Task.created_by if hasattr(Task, 'created_by') else ProjectMember.user_id))).select_from(ProjectMember).where(
            ProjectMember.project_id == project_id
        )
    )
    active_member_ratio = 80.0  # simplified
...
async def get_health(
    ...
    cached = await db.execute(
        select(ProjectHealthScore).where(ProjectHealthScore.project_id == project_id)
    )
